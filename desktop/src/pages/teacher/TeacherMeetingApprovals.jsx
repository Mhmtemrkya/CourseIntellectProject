    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_1.3fr]">
        <Card className="border-brand-primary/10 shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5 text-brand-primary" />Müsaitlik Takvimi</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={() => setOnlineMeeting(true)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${onlineMeeting ? 'bg-brand-primary text-white' : 'bg-slate-100 text-slate-600'}`}>
                <Video className="mr-2 inline h-4 w-4" />Online
              </button>
              <button type="button" onClick={() => setOnlineMeeting(false)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${!onlineMeeting ? 'bg-brand-primary text-white' : 'bg-slate-100 text-slate-600'}`}>
                <Building2 className="mr-2 inline h-4 w-4" />Yüz yüze
              </button>
            </div>
            <div className="space-y-3">
              {groupedAvailability.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-brand-primary/10 bg-brand-primary/5 p-8 text-center text-sm text-slate-500">
                  Henüz takvime eklenmiş görüşme saati yok.
                </div>
              ) : groupedAvailability.map((group) => (
                <div key={group.dateKey} className="rounded-[24px] border border-brand-primary/10 bg-brand-primary/5 p-4">
                  <p className="font-semibold text-slate-900">{group.dayLabel}</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {group.slots.map((slot) => (
                      <div key={slot.id || slot.raw} className="inline-flex items-center gap-2 rounded-full border border-brand-primary/10 bg-white px-3 py-2 text-sm text-slate-700">
        ...
        <Card className="border-brand-primary/10 shadow-sm">
          ...
          <CardContent className="space-y-4">
            {requestCards.length === 0 ? (
              ...
              <div key={item.id} className="rounded-[28px] border border-brand-primary/10 bg-white p-5 shadow-sm">

  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="relative overflow-hidden rounded-[32px] bg-[linear-gradient(135deg,var(--brand-primary-hex)_0%,var(--brand-accent-hex)_100%)] p-8 text-white shadow-[0_24px_80px_rgba(15,23,42,0.18)]">
        <div className="absolute right-6 top-6 rounded-full bg-foreground/10 px-4 py-2 text-sm font-semibold backdrop-blur">Haftalık Rapor Merkezi</div>
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-4">
            <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-foreground/12">
            ...
            <div className="flex flex-wrap gap-3">
              <Button onClick={openComposer} className="bg-white text-slate-900 hover:bg-foreground/90">
                ...
                <div className="flex flex-col gap-5 p-5 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="grid shrink-0 gap-2 text-right">
                    ...
                    <div className="rounded-2xl bg-brand-primary px-4 py-3 text-white">
                      <div className="text-xs uppercase tracking-[0.18em] text-foreground/60">Ekler</div>
          ...
          <CardContent className="space-y-5 p-5">
            ...
            <div className="rounded-[24px] bg-slate-950 p-5 text-white">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.22em] text-foreground/60">Son gönderim</p>
          ...
          <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="space-y-4">
              <div className="rounded-[28px] bg-[linear-gradient(135deg,var(--brand-primary-hex)_0%,var(--brand-accent-hex)_100%)] p-5 text-white">
                <div className="text-xs uppercase tracking-[0.2em] text-foreground/55">Veliye gidecek önizleme</div>
                <h3 className="mt-3 text-2xl font-bold">{form.title.trim() || `${form.subject || 'Ders'} Haftalık Raporu`}</h3>
                <p className="mt-2 text-sm text-foreground/70">{selectedStudent?.fullName || 'Öğrenci seçilmedi'} • {form.className || 'Sınıf'} • {form.weeklyPeriodLabel}</p>
                <div className="mt-5 rounded-2xl bg-foreground/10 p-4 text-sm leading-6 text-foreground/82">
          ...
          {selectedReport ? (
            <div className="space-y-5">
              <div className="rounded-[28px] bg-[linear-gradient(135deg,var(--brand-primary-hex)_0%,var(--brand-accent-hex)_100%)] p-6 text-white">
                <div className="flex flex-wrap items-center gap-2">
                  <Badge className="bg-foreground/12 text-white hover:bg-foreground/12">{selectedReport.subject}</Badge>
                  <Badge className="bg-foreground/12 text-white hover:bg-foreground/12">{selectedReport.weeklyPeriodLabel}</Badge>
                </div>
                <p className="mt-4 text-lg leading-8 text-foreground/88">{selectedReport.summary}</p>
              </div>
...
function StatGlassCard({ icon: Icon, label, value }) {
  return (
    <div className="rounded-[26px] border border-foreground/10 bg-foreground/10 p-5 backdrop-blur">

      ...
      <Card className="overflow-hidden border-0 bg-gradient-to-br from-slate-950 via-slate-900 to-brand-accent text-white shadow-xl">
        <CardContent className="grid gap-6 p-6 lg:grid-cols-[1.4fr_1fr]">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full bg-foreground/12 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-foreground/90">
              <Sparkles className="h-3.5 w-3.5" />
              Yoklama merkezi
            </div>
            <div>
              <h2 className="text-2xl font-bold sm:text-3xl">{selectedLessonSummary}</h2>
              <p className="mt-2 max-w-2xl text-sm text-foreground/72 sm:text-base">
            ...
            <div className="flex flex-wrap gap-3">
              <div className="rounded-2xl bg-foreground/10 px-4 py-3 backdrop-blur">
                <p className="text-[11px] uppercase tracking-[0.2em] text-foreground/70">Secili ders</p>
                <p className="mt-1 font-semibold">{selectedLesson?.lesson || 'Ders secin'}</p>
              </div>
              <div className="rounded-2xl bg-foreground/10 px-4 py-3 backdrop-blur">
                <p className="text-[11px] uppercase tracking-[0.2em] text-foreground/70">Sinif</p>
                <p className="mt-1 font-semibold">{selectedLesson?.className || '-'}</p>
              </div>
              <div className="rounded-2xl bg-foreground/10 px-4 py-3 backdrop-blur">
                <p className="text-[11px] uppercase tracking-[0.2em] text-foreground/70">Yoklama orani</p>
                <p className="mt-1 font-semibold">%{attendanceRate}</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Card className="border-foreground/10 bg-foreground/10 text-white shadow-none"><CardContent className="p-4"><div className="flex items-center gap-3"><Users className="h-5 w-5 text-cyan-200" /><div><p className="text-xs text-foreground/70">Ogrenci</p><p className="text-2xl font-bold">{lessonStudents.length}</p></div></div></CardContent></Card>
            <Card className="border-foreground/10 bg-foreground/10 text-white shadow-none"><CardContent className="p-4"><div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-emerald-300" /><div><p className="text-xs text-foreground/70">Geldi</p><p className="text-2xl font-bold">{counts.present}</p></div></div></CardContent></Card>
            <Card className="border-foreground/10 bg-foreground/10 text-white shadow-none"><CardContent className="p-4"><div className="flex items-center gap-3"><Clock3 className="h-5 w-5 text-amber-200" /><div><p className="text-xs text-foreground/70">Gec</p><p className="text-2xl font-bold">{counts.late}</p></div></div></CardContent></Card>
            <Card className="border-foreground/10 bg-foreground/10 text-white shadow-none"><CardContent className="p-4"><div className="flex items-center gap-3"><XCircle className="h-5 w-5 text-rose-200" /><div><p className="text-xs text-foreground/70">Gelmedi</p><p className="text-2xl font-bold">{counts.absent}</p></div></div></CardContent></Card>
          </div>
        </CardContent>
      </Card>
      <Card>

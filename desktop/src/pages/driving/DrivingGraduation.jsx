...
export default function DrivingGraduation() {
  ...
  return <DrivingPage testId="driving-graduation-page">
    ...
    <div className="grid gap-4 xl:grid-cols-2">{data.students.map((student) => {
      ...
      return <Card key={student.id}><CardHeader><CardTitle className="flex items-center justify-between"><span>{student.fullName}</span><Badge>{graduation?.status || student.status}</Badge></CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        {checklist && <div className="space-y-2 rounded-xl border p-3">{checklist.items.map((item) => <div key={item.key} className="flex gap-2 text-sm">{item.completed ? <CheckCircle2 className="h-5 w-5 text-emerald-500" /> : <Circle className="h-5 w-5 text-amber-500" />}<div><b>{item.label}</b><p className="text-muted-foreground">{item.detail}</p></div></div>)}
          <div className="flex flex-wrap gap-2">{can(DRIVING.graduationManage) && !graduation?.graduatedAtUtc && <Button disabled={saving} onClick={() => run(() => graduateDrivingStudent(student.id, 'Kontrol listesi veya iki onaylı istisna tamamlandı.'), 'Kursiyer mezun edildi')}><Award className="mr-2 h-4 w-4" />Mezun Et</Button>}
        ...
        {graduation?.status === 'Graduated' && <div className="space-y-2 rounded-xl bg-emerald-500/5 p-3"><b>Mezuniyet: {new Date(graduation.graduatedAtUtc).toLocaleDateString('tr-TR')}</b>
          {can(DRIVING.certificateIssue) && certificates.length === 0 && <div className="flex gap-2"><Button size="sm" onClick={() => run(() => issueDrivingCertificate(student.id, 'Completion'), 'Tamamlama belgesi oluşturuldu')}><FileBadge2 className="mr-1 h-4 w-4" />Tamamlama Belgesi</Button><Button size="sm" variant="outline" onClick={() => run(() => issueDrivingCertificate(student.id, 'Achievement'), 'Başarı belgesi oluşturuldu')}>Başarı Belgesi</Button></div>}
        ...
        {requests.map((request) => <div key={request.id} className="rounded-xl border border-amber-300/60 bg-amber-500/5 p-3 text-sm"><b>{request.actionType} • {request.status}</b><p>{request.reason}</p>{can(DRIVING.graduationOverrideApprove) && ['Pending','FirstApproved'].includes(request.status) && <div className="mt-2 flex gap-2"><Button size="sm" onClick={() => run(() => approveDrivingGraduationAction(request.id, 'Kontrol edilerek onaylandı.'), 'Onay kaydedildi')}>Onayla</Button><Button size="sm" variant="outline" onClick={() => { const note = window.prompt('Ret gerekçesi:'); if (note?.trim().length >= 10) run(() => rejectDrivingGraduationAction(request.id, note), 'Talep reddedildi'); }}>Reddet</Button></div>}</div>)}

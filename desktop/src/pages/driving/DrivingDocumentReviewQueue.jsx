  ...
  return <DrivingPage testId="driving-document-review-queue-page">
    ...
    {items.length === 0 ? <DrivingNotice icon={CheckCircle2} title="Bu filtrede belge yok." message="Evrak kuyruğu güncel." /> : <div className="space-y-3">{items.map((item) => {
      ...
      return <section key={item.id} className="rounded-2xl border p-4">
        ...
        {canReview && <div className="mt-4 flex flex-wrap gap-2"><Button disabled={busy} className="bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => act(item, 'Approve')}><CheckCircle2 className="mr-2 h-4 w-4" />Onayla</Button><Button disabled={busy || values.reason.trim().length < 5} variant="destructive" onClick={() => act(item, 'Reject')}><XCircle className="mr-2 h-4 w-4" />Reddet</Button><Button disabled={busy || values.reason.trim().length < 5} variant="outline" onClick={() => act(item, 'RequestReupload')}><RotateCcw className="mr-2 h-4 w-4" />Yeniden yükleme iste</Button><Button disabled={busy} variant="outline" onClick={() => act(item, 'UpdateDetails')}>Tarih/notu kaydet</Button></div>}
      </section>;
    })}</div>}
  </DrivingPage>;
}

...
function StructuredData({ message, onAction }) {
  ...
  return <div className="mt-3 space-y-2">
    ...
    {typeof data.collectedThisMonth === 'number' && <div className="grid grid-cols-1 gap-2">
      <div className="rounded-xl border bg-background/80 p-3"><div className="text-xs text-muted-foreground">{data.periodLabel} tahsilat</div><div className="text-2xl font-bold text-foreground">{data.collectedThisMonth.toLocaleString('tr-TR')} ₺</div></div>
      <div className="flex gap-2"><div className="flex-1 rounded-xl border bg-background/80 p-3"><div className="text-xs text-muted-foreground">Açık borç</div><div className="text-lg font-bold text-foreground">{(data.openDebt ?? 0).toLocaleString('tr-TR')} ₺</div></div><div className="flex-1 rounded-xl border bg-background/80 p-3"><div className="text-xs text-muted-foreground">Gecikmiş taksit</div><div className="text-lg font-bold text-foreground">{data.overdueInstallments ?? 0}</div></div></div>
    </div>}

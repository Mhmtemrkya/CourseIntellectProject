...
class AccountingReceiptArchivePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final items = [...store.collections]..sort((a, b) => b.time.compareTo(a.time));
      ...
      child: ListView(
        ...
        children: [
          AccountingHeroCard(
            ...
            title: 'Canlı tahsilat kayıtlarından üretilen makbuzlar burada listelenir.',
            description: 'Her satır son tahsilat zamanı, öğrenci ve ödeme yöntemiyle birlikte tutulur.',
          ...
          if (items.isEmpty)
            const AccountingPanel(child: Text('Henüz makbuz arşivine düşen tahsilat bulunmuyor.'))
          else
            ...items.map(
              (item) => AccountingPanel(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(item.name, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w900)),
                    const SizedBox(height: 4),
                    Text('${item.className} • ${item.method}', style: Theme.of(context).textTheme.bodySmall),
                    const SizedBox(height: 8),
                    Text('${item.amount} • ${item.time}', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w700)),
                  ],
                ),
              ),
            ),

...
class _VeliExamResultsPageState extends State<VeliExamResultsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final trialRecords = allRecords
        .where((item) => item.type == 'Deneme')
        .toList();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '${_decodeText(_studentName)} için deneme, yazili ve ders bazlı tüm sonuçlar profesyonel özetle izlenir.',
  ...
  Widget _segmentBar() {
    final segments = ['Tümu', 'Deneme', 'Yazılı', 'Quiz'];
  ...
  Widget _summaryGrid(
    ...
    List<ExamScoreRecord> allRecords,
    List<ExamScoreRecord> trialRecords,
  ) {
    final bestRecords = [...allRecords]
        ...
        children: [
          Expanded(
            child: _summaryCard(
              ...
              subtitle: best == null
                  ? 'Kayıt yok'
                  : '${_decodeText(best.subject)} • ${_decodeText(best.examTitle)}',
  ...
  Widget _lessonSection(
    BuildContext context,
    Map<String, List<ExamScoreRecord>> lessonGroups,
  ) {
    return _surfaceSection(
      context,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Ortalama $avg • Son kayıt ${_decodeText(last.type)}',
  ...
  List<ExamScoreRecord> _filterRecords(List<ExamScoreRecord> records) {
    if (_selectedSegment == 'Tümu') {
      return records;
    }
    return records.where((item) => item.type == _selectedSegment).toList();
  }

  Map<String, List<ExamScoreRecord>> _groupBySubject(
    List<ExamScoreRecord> records,
  ) {

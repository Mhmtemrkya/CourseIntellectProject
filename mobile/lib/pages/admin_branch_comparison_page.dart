...
class AdminBranchComparisonPage extends StatefulWidget {
  ...
  @override
  State<AdminBranchComparisonPage> createState() => _AdminBranchComparisonPageState();
}
...
class _AdminBranchComparisonPageState extends State<AdminBranchComparisonPage> {
  ...
  Widget build(BuildContext context) {
    final campuses = StaffRegistryStore.instance.staff
    ...
    final branches = campuses.map((campus) {
      final staffCount =
          StaffRegistryStore.instance.staff.where((item) => (item.campus.isEmpty ? 'Belirtilmedi' : item.campus) == campus).length;
    ...
    return AdminScaffold(
      appBar: AppBar(title: const Text('Sube Karsilastirma', style: TextStyle(fontWeight: FontWeight.bold))),
      child: ListView(
              ...
              (branch) => AdminPanel(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text(branch.$1, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w900)),
                        const Spacer(),
                        AdminAccentBadge(label: branch.$2, color: branch.$5),
                      ],
                    ),
                    const SizedBox(height: 10),
                    Text('${branch.$3} • ${branch.$4}', style: Theme.of(context).textTheme.bodyMedium),
                  ],
                ),
              ),
            )

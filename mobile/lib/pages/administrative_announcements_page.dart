...
class AdministrativeAnnouncementsPage extends StatefulWidget {
  ...
  @override
  State<AdministrativeAnnouncementsPage> createState() => _AdministrativeAnnouncementsPageState();
}

class _AdministrativeAnnouncementsPageState extends State<AdministrativeAnnouncementsPage> {
  bool _loading = true;
  String? _error;
      ...
      child: ListView(
        ...
        children: [
          AdminHeroCard(
            ...
            title: 'Kayıt, veli bilgilendirme ve kurum içi akışları hedefli listelerle yönetin.',
            description: 'Desktop tarafındaki gibi sınıf ve kişi seçerek yalnızca ilgili hesaplara duyuru yayınlayın.',
            colors: const [Color(0xFF0F172A), Color(0xFFB45309)],
            metrics: [
              AdminHeroMetric(label: 'Toplam', value: '${_announcements.length}'),
              AdminHeroMetric(label: 'Sınıf', value: _classes.isEmpty ? 'Hazır değil' : '${_classes.length}'),
            ],
          ),
              ...
              child: Center(
                child: Column(
                  children: [
                    ...
                    FilledButton(onPressed: _loadAnnouncements, child: const Text('Tekrar Dene')),
                  ],
                ),
              ),
            )
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(item.summaryDetail, style: Theme.of(context).textTheme.bodySmall?.copyWith(height: 1.45)),
                const SizedBox(height: 8),
                Text(item.date, style: Theme.of(context).textTheme.bodySmall?.copyWith(fontWeight: FontWeight.w700)),
              ],
            ),
          ),
        ],
      ...
      builder: (sheetContext) {
        return StatefulBuilder(
          builder: (context, setSheetState) {
            final recipientOptions = audience == 'Teacher' || audience == 'Tum Kurum'
            ...
            return PremiumModalShell(
              ...
              description: 'Sınıf ve alıcı listesini seçerek yalnızca ilgili veli veya öğrencilere yayın yap.',
              colors: const [Color(0xFF0F172A), Color(0xFFB45309)],
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    decoration: const InputDecoration(labelText: 'Hedef Kitle', border: OutlineInputBorder()),
                    items: const [
                      ...
                      DropdownMenuItem(value: 'Tum Kurum', child: Text('Tüm Kurum')),
                    ],
                  ...
                  const SizedBox(height: 12),
                  if (audience == 'Veli' || audience == 'Ogrenci') ...[
                    DropdownButtonFormField<String>(
                  ...
                  if (audience == 'Veli') ...[
                    Text('Liste Kaynağı', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
                    const SizedBox(height: 8),
                    Wrap(
                      spacing: 10,
                      runSpacing: 10,
                  ...
                  if (recipientOptions.isNotEmpty) ...[
                    ...
                    Row(
                      children: [
                        Text('Seçili Kişiler', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
                        const Spacer(),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: OutlinedButton(onPressed: () => Navigator.pop(sheetContext), child: const Text('İptal')),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: FilledButton(
                          onPressed: () async {
                            if ((audience == 'Veli' || audience == 'Ogrenci') && selectedKeys.isEmpty) {
                            ...
                            try {
                              await SchoolFeedApiService.instance.createAnnouncement(
                                title: titleController.text.trim().isEmpty ? 'Yeni idari duyuru' : titleController.text.trim(),
                                detail: detailController.text.trim().isEmpty ? 'İdari duyuru içeriği yayınlandı.' : detailController.text.trim(),
                                audience: audience,
                            ...
                            _loadAnnouncements();
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Duyuru yayınlandı.'), behavior: SnackBarBehavior.floating),
                            );
                          },

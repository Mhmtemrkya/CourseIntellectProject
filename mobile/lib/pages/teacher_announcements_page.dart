  ...
  Future<void> _loadAnnouncements() async {
    ...
    try {
      final results = await Future.wait([
        ...
        SchoolFeedApiService.instance.fetchAnnouncements(audience: 'Ogrenci'),
      ...
      final classes = <String>{
        ...
        ...((results[3] as List<dynamic>).map((item) => item.toString().trim()).where((item) => item.isNotEmpty)),
      }.toList()
        ..sort();
      ...
      child: ListView(
        ...
        children: [
          AdminHeroCard(
            ...
            description: 'Sınıf duyuruları, hatırlatmalar ve öğretmen içi bilgilendirmeler bu ekrandan yayınlanır.',
            colors: const [Color(0xFF0F172A), Color(0xFF2563EB)],
            metrics: [
              AdminHeroMetric(label: 'Toplam', value: '${_announcements.length}'),
              AdminHeroMetric(
                label: 'Öğrenci',
                value: '${_announcements.where((item) => item.audience.toLowerCase().contains('ogrenci')).length}',
              ),
              AdminHeroMetric(
                label: 'Öğretmen',
                value: '${_announcements.where((item) => item.audience.toLowerCase().contains('teacher')).length}',
              ),
            ],
          ),
        ...
        return StatefulBuilder(
          builder: (context, setSheetState) {
            ...
            return PremiumModalShell(
              ...
              description: 'Desktop akışındaki gibi sınıf seç, liste kaynağını belirle ve duyuruyu sadece seçtiğin kişilere gönder.',
              colors: const [Color(0xFF0F172A), Color(0xFF2563EB)],
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                          ...
                          children: recipientOptions.map((option) {
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(18),
                                border: Border.all(
                                  color: checked ? const Color(0xFF0F172A) : const Color(0xFFE2E8F0),
                                ),
                              ...
                              child: CheckboxListTile(
                                ...
                                title: Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    color: checked ? Colors.white : const Color(0xFF0F172A),
                                  ),
                                ),
                                subtitle: Text(
                                  option.helper,
                                  style: TextStyle(
                                    color: checked ? Colors.white70 : const Color(0xFF64748B),
                                  ),
                                ),
                          ...
                          onPressed: () async {
                            if ((audience == 'Veli' || audience == 'Ogrenci') && selectedKeys.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('Lütfen duyuruyu alacak kişileri seçin.'),
